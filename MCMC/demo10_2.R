...
df2=data.frame(x,q,w*x)%>%gather(key=grp,value = p,-x)
samp <- ggplot() +
  # geom_line(aes(x,q,color='2'))
  geom_line(data = df2,aes(x,p, color = grp,fill = grp)) +
  # geom_segment(aes(x = x[rsi], xend = x[rsi], y = 0, yend = x[rsi]*wr),
  #              alpha = 0.5, color = 'steelblue') +
  labs(title = 'Samples and importance weights', x = '', y = '') +
  # scale_color_manual(values = c('steelblue','red'), labels = 'q(theta|y)/g(theta)') +
  theme(legend.position = 'bottom', legend.title = element_blank())
